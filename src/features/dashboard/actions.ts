...
export async function startBreakAction(formData: FormData) {
  ...
  const payload: Database["public"]["Tables"]["breaks"]["Insert"] = {
    entry_id: activeEntryId,
    break_type: parsedBreakType.data,
    started_at: new Date().toISOString()
  };

  const { error } = await supabase.from("breaks").insert(payload as never);
...
export async function finishBreakAction() {
  ...
  const payload: Database["public"]["Tables"]["breaks"]["Update"] = {
